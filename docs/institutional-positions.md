...
> Per-provision detail lives in [`data/positions.csv`](../data/positions.csv) and
> [`provisions/`](provisions/); this page is the prose overview. The one-screen current state is in
> [`STATUS.md`](../STATUS.md).
...
[register](../sources/README.md)). The first formal position signals to watch (see
[`STATUS.md` milestones](../STATUS.md#next-milestones-to-watch)):

- **Council** — the first Presidency compromise text (a new ST number following the
  [ST 10094/26](../sources/README.md) cover-note stream) will mark where delegations start pulling;
  expected under the Irish Presidency (H2 2026), with a competitiveness/industry working party as
  likely lead given the COMPET-first routing. Until WK delegation-comment streams appear, the
  baseline Council-side signal is **pre-legislative**: the **Semicon Coalition Declaration** calling
  for a Chips Act 2.0, signed by all 27 Member States on 29 September 2025
