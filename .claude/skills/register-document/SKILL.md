...
1. **Gather metadata.** From the URL (WebFetch) and/or the local file, establish:
   ...
   - Apply the **scope rule** from [`tracker.yaml`](../../../tracker.yaml): in-scope only if it concerns
     this repo's file (`file_id`), not a `keep_apart` sibling. (For 2026/0139 the easy confusion is its package sibling
     CADA, file 2026/0138 / COM(2026) 502; for foreign-language sources check the file number, not just the topic.)
...
2. **Pick the source folder by `body`** and commit the file there with the repo's filename
   ...
   |---|---|---|
   | Commission | `sources/commission/` | `COM-2026-504_proposal_2026-06-03.pdf` |
   | Council (ST / WK / CM) | `sources/council/` | `ST-10094-2026_cover-note-proposal_2026-06-03.pdf` |
