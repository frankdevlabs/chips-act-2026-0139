...
2. **Gather evidence.** Fetch `source_url` (WebFetch) and compare against what the repo
   ...
   **close the issue as not-relevant** — do not edit the repo.
   - **Scope tie-breaker for EP committee docs.** A committee may opine on *both* sibling files; a
     ...
     reference on the document itself** matches **this** file (`tracker.yaml` `file_id` = `2026/0139
     (COD)`), not the `keep_apart` sibling **CADA `2026/0138 (COD)`**, and confirm the **rapporteur**
     (the same committee can have a different rapporteur on each sibling file). Cross-check OEIL for
     *both* files before deciding scope.
